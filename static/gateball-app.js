(function(){
  ...
  // authenticated chat realtime alerts, app-style Android back handling,
  // and Home-page unread Chat badge.
  ...
  function showDashboardNotificationPrompt(){
    ...
    if(!('Notification' in window)){
      ...
      text.textContent=
        '⚠️ This browser does not expose the Notification API. Open Gateball Lovers in Chrome and check Android site notification permissions.';
    ...
    if(Notification.permission === 'granted'){
      ...
      text.textContent=
        'Notifications are allowed. Registering this phone for background chat notifications...';
    ...
    if(Notification.permission === 'denied'){
      ...
      text.textContent=
        'Notifications are blocked for this browser. Enable Gateball Lovers notifications in Android/Chrome site settings, then return here.';
    ...
    text.textContent=
      'Enable notifications to receive new chat alerts even when Gateball Lovers is closed.';
    ...
    btn.onclick=function(){
      ...
      Notification.requestPermission().then(function(result){
        ...
        if(result==='granted'){
          ...
          text.textContent=
            'Notifications are blocked. Enable them in Android/Chrome site settings and return here.';
        ...
        text.textContent=
          'Could not request notification permission: '+(err.message||err);
  ...
  async function subscribeForPushIfConfigured(showTest){
    ...
    try{
      ...
      if(btn){
        btn.disabled=true;
        btn.textContent='Registering this phone...';
      }
      ...
      var vapidKey=keyData.public_key || keyData.vapid_public_key;
      ...
      var saveText=await saveResponse.text();
      ...
      if(!saveResponse.ok || !saveData.ok){
        ...
        throw new Error(
          saveData.error ||
          'Could not save this phone notification subscription.'
        );
      ...
      setPushText(
        '✅ This phone is registered for background chat notifications. Keep notifications allowed in Android/Chrome.'
      );
      ...
      setPushText(
        '❌ Push setup failed: '+pushState.lastError
      );
      ...
      if(btn){
        ...
        btn.disabled=false;
        btn.textContent='🔁 RETRY CHAT NOTIFICATIONS';
  ...
  async function sendPushTest(){
    ...
    try{
      ...
      if(!r.ok || !data.ok){
        ...
        throw new Error(
          data.error ||
          ('Push test failed ('+r.status+'). Sent: '+(data.sent||0))
        );

      }

      setPushText(
        '✅ Test push sent. If this phone is closed/backgrounded, the notification should appear in the Android notification bar.'
      );

      try{
        localStorage.setItem('gl_push_tested','1');
      }catch(e){}

      var firstNotice=
        document.getElementById('notificationPromptDashboard');
      ...
      if(btn){
        ...
        btn.disabled=false;
        btn.textContent='🔔 SEND TEST AGAIN';
      ...
      setPushText(
        '❌ Test push failed: '+(e.message||e)
      );

      if(btn){

        btn.disabled=false;
        btn.textContent='🔁 RETRY PUSH TEST';
  ...
  function showFirstTimeDashboardNotice(){
    ...
    try{
      tested=
        localStorage.getItem('gl_push_tested')==='1';
    }catch(e){}
  ...
  /*
   * ============================================================
   * HOME PAGE CHAT UNREAD BADGE
   * ============================================================
   */

  function updateDashboardChatBadge(total){
    ...
    var el=
      document.getElementById('dashboardChatBadge');
  ...
  async function loadDashboardChatBadge(sb,myId){
    ...
    try{
      ...
      updateDashboardChatBadge(total);
  ...
  async function initGlobalChatNotifications(){
    ...
    try{
      ...
      /*
       * Realtime private-message listener.
       */
      var channel=
        sb
          .channel(
          ...
          .on(
            ...
            function(payload){
              ...
              /*
               * If currently on Home, immediately
               * increase the Chat badge.
               */
              if(
                location.pathname === '/dashboard'
              ){
